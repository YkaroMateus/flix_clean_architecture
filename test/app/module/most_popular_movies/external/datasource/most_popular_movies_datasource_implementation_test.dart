...
import 'package:flix_clean_ark/app/module/most_popular_movies/external/datasource/most_popular_movies_datasource_implementation.dart';
...
void main() {
  ...
  test('Must return an MostPopularMoviesModel on status code 200', () async {
    when((() => dio.get(any()))).thenAnswer((invocation) async => Response(
        data: mostPopularMoviesSucessResponse, statusCode: 200, requestOptions: RequestOptionsFake()));

    final result = await datasource();
    expect(result, isA<MostPopularMoviesModel>());
  });

  test('Must return an MostPopularMoviesDatasourceError on any other status code', () async {
    when((() => dio.get(any()))).thenAnswer(
        (invocation) async => Response(data: '', statusCode: 400, requestOptions: RequestOptionsFake()));

    final result = datasource();
    expect(result, throwsA(isA<MostPopularMoviesDatasourceError>()));
  });

  test('Must return an Exception on dio error ', () async {
